...
public class MySql5PageHepler {
    ...
    protected final static Log logger = LogFactory.getLog(MySql5PageHepler.class);

    /**
     * 得到SQL第一个正确的FROM的的插入点。
     *
     * @param querySelect 查询语句。
     */
    ...
    public static String addLimitString(String querySelect, int offset, int limit) {
        ...
        if(logger.isDebugEnabled()){
            logger.debug("addLimitString offset=" + offset + ",limit=" + limit);
        }
        ...
        Statement stmt = null;
        try {
            stmt = CCJSqlParserUtil.parse(querySelect);
        } catch (JSQLParserException e) {
            return querySelect + " limit " + offset + " ," + limit;
        }
        Select select = (Select) stmt;
        SelectBody sBody = select.getSelectBody();
        final String currentSql = querySelect.substring(0, querySelect.indexOf("FROM") + 5);
        CustomSelectVisitor selectVisitor = new CustomSelectVisitor(currentSql, offset, limit);
        sBody.accept(selectVisitor);
        sql = selectVisitor.getSql();
